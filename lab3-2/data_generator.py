...
def gen_one_case(i, in_fd=None, c_fd=None, all_one=False, mode=0, shape_range=(4, 255), a_val_range=(0, 16), x_val_range=(0, 255)):
    ...
    if mode == 0:
        ...
        M, K = FIXED_GEMV_SHAPES[i % len(FIXED_GEMV_SHAPES)]
    ...
    write_matrix(in_fd, Bv.reshape(K, 1))
    write_output_matrix(in_fd, Cv.reshape(M, 1))
    ...
    write_readable(c_fd, Bv.reshape(K, 1), desc ="x: \n")
    write_readable(c_fd, Cv.reshape(M, 1), desc="y: \n")
